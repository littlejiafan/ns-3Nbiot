...
void
LteUeRrc::NbDoForceCampedOnEnb (uint16_t cellId, uint16_t dlEarfcn,uint16_t indexx)
{
  NS_LOG_FUNCTION (this << m_imsi << cellId << dlEarfcn<<indexx);
  ...
  switch (m_state)
    ...
    case IDLE_START:
      ...
      m_dlEarfcn = dlEarfcn;
      m_index=indexx;
      m_cphySapProvider->NbSynchronizeWithEnb (m_cellId, m_dlEarfcn,m_index);
      SwitchToState (IDLE_WAIT_MIB);
      break;
