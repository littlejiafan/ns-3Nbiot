...
void
EpcUeNas::NbConnect (uint16_t cellId, uint16_t dlEarfcn,uint16_t indexx)
{
  NS_LOG_FUNCTION (this << cellId << dlEarfcn<<indexx);

  // force the UE RRC to be camped on a specific eNB
  m_asSapProvider->NbForceCampedOnEnb (cellId, dlEarfcn,indexx);
...
EpcUeNas::ActivateEpsBearer (EpsBearer bearer, Ptr<EpcTft> tft)
{
  ...
  switch (m_state)
    ...
    default:
      ...
      std::cout<<"无线承载建立，附着流程结束"<<std::endl;;
      break;
    }
}
