...
PointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  ...
  std::cout<<"这里是P2P-EPC-HELPER,正在激活对MME的无线承载..."<<std::endl;
...
uint8_t
PointToPointEpcHelper::NbActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  NS_LOG_FUNCTION (this << ueDevice << imsi);
  ...
  // we couldn't do it before since address assignment is triggered by
  // the user simulation program, rather than done by the EPC
  std::cout<<"这里是P2P-EPC-HELPER,NBIOT正在激活对MME的无线承载..."<<std::endl;
  Ptr<Node> ueNode = ueDevice->GetNode ();
  Ptr<Ipv4> ueIpv4 = ueNode->GetObject<Ipv4> ();
  NS_ASSERT_MSG (ueIpv4 != 0, "UEs need to have IPv4 installed before EPS bearers can be activated");
  int32_t interface =  ueIpv4->GetInterfaceForDevice (ueDevice);
  ...
  uint8_t ueAbbility=ueDevice->getIndex();

  uint8_t bearerId = m_mme->NbAddBearer (imsi, tft, bearer, ueAbbility);//看来这一步很关键！
